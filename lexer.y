...
%%
program: declaration-list{startCode($1);}  | %empty;
...
| VAR ID more-vars DECLARE data-type EOL {node* t1=makeTree2("id's",makenode($2),$3);node* type1 =makeTree1("type",$5);$$=makeTree2("vardeclare",t1,type1);}|
...
WHILE '(' expr ')' '{''}';
more-vars:  more-vars COMMA ID{$$=makeTree1($1,$3);} |
COMMA ID {$$=makenode($2);}| 
%empty {$$=makenode("");};
